...
export default async function handler(req, res) {
    ...
    const keys = dates.flatMap((date) => [`stats:downloads:${date}`, `stats:visits:${date}`]);
    ...
    return res.status(200).json({
        totals: { downloads: Number(totals[0] || 0), visits: Number(totals[1] || 0) },
        today: todayStats,
        yesterday: yesterdayStats,
        last7Days: series.slice(-7).reduce((sum, item) => ({
            downloads: sum.downloads + item.downloads,
            visits: sum.visits + item.visits,
        }), { downloads: 0, visits: 0 }),
        last30Days: series.reduce((sum, item) => ({
            downloads: sum.downloads + item.downloads,
            visits: sum.visits + item.visits,
        }), { downloads: 0, visits: 0 }),
        series,
